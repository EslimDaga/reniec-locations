...
const LocationCard = ({ location }: LocationCardProps) => {
  if (!location) {
    return (
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[480px] bg-white/9 rounded-3xl shadow-2xl border border-zinc-200/50 backdrop-blur-md overflow-hidden z-50 animate-fade-up h-auto">
        <div className="px-8 py-10 text-center space-y-6">
          <div className="relative">
            <div
              className="absolute -top-1 left-1/2 -translate-x-1/2 w-24 h-24 bg-emerald-100/50
                          rounded-full blur-xl"
            />
            <div
              className="relative mx-auto w-16 h-16 bg-emerald-50 rounded-2xl
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[480px] max-h-[46vh] bg-white/95 rounded-3xl shadow-2xl border border-zinc-200/50 backdrop-blur-md overflow-hidden z-50 animate-slide-up">
      {/* Header */}
      <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-zinc-100/50 flex items-center justify-between p-5">
        <div className="flex items-center gap-3 justify-between w-full">
          ...
          <span
            className={`px-2 py-0.5 font-medium rounded-full whitespace-nowrap ${
